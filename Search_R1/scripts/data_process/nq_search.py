...
def make_prefix(dp, template_type):
    ...
    if template_type == 'base':
        ...
        prefix = f"""请根据以下要求回答问题：
每次获取新信息时，必须先在 `<think>` 和 `</think>` 标签内进行推理。
完成推理后，如果发现缺少相关知识，可以通过 `<search> 查询词 </search>` 调用搜索引擎，搜索结果将以 `<information>` 和 `</information>` 的形式返回。
你可以不限次数地进行搜索。
如果认为不需要外部知识，则直接在 `<answer>` 和 `</answer>` 标签内给出最终答案，无需详细说明和进行 `<search>` 操作，例如：`<answer>北京</answer>`。
问题：{question}\n"""
    else:
